...
** Otherwise, keep including current index. Keep updating best answer
   at every point
** The following impl gives the max-sum assuming its positive, otherwise
...
* Now when we ahve a key, we go to the circle and pick the node that appears 
  next in the clock-wise direction.

Thus when a node leaves, since its present in mulitiple points in the circle,
...
* Strongly connected: If you can reach any vertex from any other vertex, following the
                      edges in thier given direction.
...
** Pick any first node. Now sort the other edges that reach from this vector
   and build the heap. Nodes that arent' directly reachable are value inf.
** Pick the cheapest edge. The thing is once you pick, the vertices values
   change as they are now even more reacheable.
...
** so, its O(|E|log(|V|)). THere is a heapify up front, but thats O(|V|).
   So we can ignore that.


=== Kruskal's Algorithm

Find the minimu spanning tree

* pick the cheapest edge as long as its
  not creating cycles, i.e it shoudln't connect
...
For an arbitraty size, the utmost n/(2-power-r) objects with rank r.
...
Weight-Independant Set in Path Graph
--------------------------------------
...
* Identiy a small number of sub-problems
* Correctly solve the bigger problem given the solutions of the sub-problem
* Ensure, the final solution is availabe by solving the sub-problems
...
* We can't greedily pick the top occuring as root - as that may be a sub-optimal choice.
  Consider 1(0.01), 2(0.34), 3(0.33), 4(0.32). Here 3 should be root, although 2 is
  indeed the highest occuring frequency.

Solution:

* The trick here is to pick the root. Assume if root is correct, then Left and right sides have to be
  optimal binary trees themselves. Thus we need to build optimal binary trees for every continguous
  subset in the given input. And we evaulate the cost of picking every element as root.
...
** Either the old soln itself is better.
** Or one of the indegree is better.

Running time:
* n iterations (1 for each edge addition from none to max(n-1))
* In each iteration, we have upto n * (in-degree) lookups. This
  n * (in-degree-of-each-ertex) is essentially the number of
...
Space optimization:
* By defin, we need O(n-square) space.
* But note that we can discard a prev iteration, ater we update all
